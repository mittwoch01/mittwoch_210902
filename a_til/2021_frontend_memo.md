...
- [ ] 변수 선언 - 호이스팅현상
- [ ] 형타입 체크 - 숫자, 문자, 특수(undefined,/null),논리(t/f),객체(o,a),함수(function)
  - [ ] 배열 메소드 : push, pop, unshift,shift
  - [ ] 참조변수(배열,객체) / 일반변수 : 값 자체
  - [ ] 함수 : 선언식-기명함수 : 함수에 이름을 가진다.(함수자체가 호이스팅)  /  표현식-익명함수 : 변수에 함수를 담는다.(변수이름만 호이스팅)
  - [ ] 즉시실행함수 : (function(매개변수){인수} ) - 선언과 동시에 실행 / 메모리 낭비를 줄이기 위해, (개인정보 등) 외부에 노출되지않고, 기능을 각각 별도로 사용하려고
  - [ ] 전역변수, 지역변수
  - [ ] 문자 + 문자 -> 문자,  문 + 숫 -> 문,  숫 + 숫 ->숫자연산,  문자/숫자 -> NaN(숫자)
...
- 변수 , Casting, list, dictionary, @mixin .....
