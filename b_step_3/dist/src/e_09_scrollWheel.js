...
const MAX_SCROLL = TOTAL_HEIGHT - WIN_HEIGHT;
const maxLen = Math.ceil(TOTAL_HEIGHT / WIN_HEIGHT);

let scrollDelta = 0; // 휠방향
let scrollIndex = 0; // 휠이 움직일 때 마다 +1 or -1 하는 것
let PERMISSION = true;
let moveScroll = WIN_HEIGHT * scrollIndex;


// 함수 ----------------------------------------------------
// 마우스 휠을 움직였을 때 수치가 1씩 증가/감소 하게하고
// 이때, 전체 길이 기준 작은 수치일 경우에 일정 크기만큼 이동하게 처리
// 중복 수행을 방지하기 위해 임의 변수 PERMISSON을 첨부하여 해당 기능을 제한 
window.scrollTo({ top: moveScroll, behavior: 'smooth'});

const fnScrollMove = ()=>{
  if(PERMISSION && scrollIndex <= maxLen){
    PERMISSION = false;
    ...
    let moveScroll = WIN_HEIGHT & scrollIndex;
    window.scrollTo({ top:WIN_HEIGHT * scrollIndex, behavior:'smooth'});
